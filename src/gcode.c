...
uint8_t gc_execute_line(char *line) 
{
    ...
    switch(letter) {
      ...
      case 'G':
        // Determine 'G' command and its modal group
				rtn = uarm_execute_g_cmd( int_value, line+char_counter, &char_counter );
      ...
      case 'M':
        ...
        if (mantissa > 0) { FAIL(STATUS_GCODE_COMMAND_VALUE_NOT_INTEGER); } // [No Mxx.x commands]
				...
				rtn = uarm_execute_m_cmd( int_value, line+char_counter, &char_counter );
				if( (rtn != UARM_CMD_NOTFIND) && (rtn != UARM_SYS_ABORT) ){ return rtn; }
      ...
      case 'P':
				if (mantissa > 0) { FAIL(STATUS_GCODE_COMMAND_VALUE_NOT_INTEGER); } // [No Mxx.x commands]
				rtn = uarm_execute_p_cmd( int_value, line+char_counter, &char_counter );
				if( (rtn != UARM_CMD_NOTFIND) && (rtn != UARM_SYS_ABORT) ){ return rtn; }
  ...
  */ 
  ...
  uint8_t rtn_res = (uint8_t)STATUS_OK;	
  switch(gc_block.non_modal_command) {
    case NON_MODAL_SET_COORDINATE_DATA:    
    ...
    case NON_MODAL_GO_HOME_0: case NON_MODAL_GO_HOME_1: 
      ...
      if (axis_command) {
          ...
          rtn_res = (uint8_t)mc_line(MOTION_MODE_LINEAR, gc_block.values.xyz, gc_state.feed_rate, false, gc_state.line_number);
        #else
          rtn_res= (uint8_t)mc_line(MOTION_MODE_LINEAR, gc_block.values.xyz, gc_state.feed_rate, false);
        #endif
      }
      #ifdef USE_LINE_NUMBERS
        rtn_res = (uint8_t)mc_line(MOTION_MODE_LINEAR, parameter_data, gc_state.feed_rate, false, gc_state.line_number); 
      #else
        rtn_res = (uint8_t)mc_line(MOTION_MODE_LINEAR, parameter_data, gc_state.feed_rate, false); 
      #endif
  ...
  if (gc_state.modal.motion != MOTION_MODE_NONE) {
    if (axis_command == AXIS_COMMAND_MOTION_MODE) {
      switch (gc_state.modal.motion) {
        ...
        case MOTION_MODE_LINEAR:
          #ifdef USE_LINE_NUMBERS
            rtn_res = (uint8_t)mc_line(MOTION_MODE_LINEAR, gc_block.values.xyz, gc_state.feed_rate, gc_state.modal.feed_rate, gc_state.line_number);
          #else
            rtn_res = (uint8_t)mc_line(MOTION_MODE_LINEAR,gc_block.values.xyz, gc_state.feed_rate, gc_state.modal.feed_rate);
          #endif
          break;
