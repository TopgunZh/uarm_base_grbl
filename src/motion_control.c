...
// in the planner and to let backlash compensation or canned cycle integration simple and direct.
  ...
  uint8_t mc_line(uint8_t mode, float *target, float feed_rate, uint8_t invert_feed_rate, int32_t line_number)
#else
  uint8_t mc_line(uint8_t mode, float *target, float feed_rate, uint8_t invert_feed_rate)
#endif
{
  ...
  delta_coord[Z_AXIS] = (target[Z_AXIS] - current_coord[Z_AXIS]) / divide_numbers; 
    ...
    final_coord[Z_AXIS] = current_coord[Z_AXIS] + delta_coord[Z_AXIS]  * i; 
		...
		do {
	      ...
	      //report_parse_result();
	      if (sys.abort) { return UARM_SYS_ABORT; } // Bail, if system abort.
...
#endif
{
  ...
  if (segments) { 
    ...
    */
    ...
    for (i = 1; i<segments; i++) { // Increment (segments-1).
      ...
      #ifdef USE_LINE_NUMBERS
        mc_line(1, position, feed_rate, invert_feed_rate, line_number);
      #else
        mc_line(1, position, feed_rate, invert_feed_rate);
      #endif
  ...
  #ifdef USE_LINE_NUMBERS
    mc_line(1, target, feed_rate, invert_feed_rate, line_number);
  #else
    mc_line(1, target, feed_rate, invert_feed_rate);
  #endif
}
